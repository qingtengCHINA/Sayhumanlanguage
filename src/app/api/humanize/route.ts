...
const SYSTEM_PROMPT = `你是一位文字编辑，专门识别和去除 AI 生成文本的痕迹，使文字听起来更自然、更有人味。
...
**只输出改写后的文本。** 不要输出改写总结、质量评分、评语、所做更改说明，或任何其他附加内容。直接输出改写后的文章正文。
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: "服务器未配置 API Key，请在 Vercel 环境变量中设置 API_KEY" },
        { status: 500 }
      );
    }
